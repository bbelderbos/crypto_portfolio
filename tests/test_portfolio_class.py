...
def create_user_and_class_instance():
    ...
    pt = Portfolio(user)
    PortfolioHoldings.objects.create(
        coin_ticker="NEO",
        number_of_coins="51",
        amount_in_usd="2000.00",
        coin_name="neo",
        type="Buy",
        person=user,
    )
...
def test_no_user_coins(coins):
    ...
    for coin in coins:
        pt.no_user_coins(
            coin.ticker, coin.num_coins, coin.usd_amt, coin.coin_name, coin.type_
        )
...
@pytest.mark.parametrize(
    "ticker, num_coins, usd_amt, coin_name, type_",
    [("ICX", 26000.326, 52000.35, "icon", "Buy"), ("DOT", 300, 9000, "DOT", "Sell")],
)
@pytest.mark.django_db(transaction=True)
...
@pytest.mark.parametrize(
    "ticker, coin_amt, usd_amt, coin_name, type_",
    [("ICX", 26000.326, 52000.35, "icon", "Buy"), ("DOT", 300, 9000, "DOT", "Sell")],
)
@pytest.mark.django_db(transaction=True)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            35000,
            Fields(
                ticker="RSR", num_coins=200, usd_amt=15000, coin_name="neo", type_="Buy"
            ),
        ),
    ],
)
